...
async def send_latest_data(websocket, path):
    try:
        while True:
            # Fetch the latest data from VehicleC table in the database
            cursor.execute("SELECT * FROM VehicleC ORDER BY timestamp DESC LIMIT 1")
...
async def send_historical_data(websocket, path):
    ...
    cursor.execute("SELECT * FROM VehicleC")
...
start_server_latest_data = websockets.serve(send_latest_data, "localhost", 8002)
start_server_historical_data = websockets.serve(send_historical_data, "localhost", 8006)
