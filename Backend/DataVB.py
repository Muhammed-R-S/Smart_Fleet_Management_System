...
async def send_latest_data(websocket, path):
    try:
        while True:
            # Fetch the latest data from VehicleB table in the database
            cursor.execute("SELECT * FROM VehicleB ORDER BY timestamp DESC LIMIT 1")
...
async def send_historical_data(websocket, path):
    ...
    cursor.execute("SELECT * FROM VehicleB")
...
start_server_latest_data = websockets.serve(send_latest_data, "localhost", 8001)
start_server_historical_data = websockets.serve(send_historical_data, "localhost", 8005)
